...
test('Berdychiv keeps street, neighborhood, and monastery owners distinct', () => {
  ...
  assert.equal(resolveLexiconGeoEntity({ country: 'UA', city: 'Berdychiv', type: 'street', canonical: 'Korolenka Street' })?.osm?.id, 193860284);
...
test('Bilhorod-Dnistrovskyi retains street and exact landmark owners', () => {
  assert.equal(resolveLexiconGeoEntity({ country: 'UA', city: 'Bilhorod-Dnistrovskyi', type: 'street', canonical: 'Portovyi Lane' })?.osm?.id, 413109017);
