...
test('Kharkiv listing canonical reuses the verified Zhukovskoho owner', () => {
  assert.equal(
...
test('Odesa listing canonicals reuse broader verified locality owners', () => {
  ...
  for (const canonical of ['Center', 'Zolotyi Bereh', 'Zastava-1', 'Zastava-2']) {
    assert.equal(isUaOdesaCoverageGap({ country: 'UA', city: 'Odesa', type: 'microdistrict', canonical }), true, canonical);
  }
  assert.equal(UA_ODESA_COVERAGE_GAPS.length, 4);
});
...
test('Samarkand legacy parser buckets reuse verified physical owners', () => {
  assert.equal(
