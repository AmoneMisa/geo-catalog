...
function slug(value) { return String(value).normalize('NFKD').replace(/\p{M}/gu, '').toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/^-+|-+$/g, ''); }
...
const previousGenerated = generatedModule[exportName(options.country)] || [];
...
const semanticDeduped = [...new Map(entities.map((entity) => [`${entity.country}|${entity.parentId}|${entity.type}|${slug(entity.canonicalName)}`, entity])).values()]
  .filter((entity) => !existingSemantic.has(`${entity.country}|${entity.parentId}|${entity.type}|${normalize(entity.canonicalName)}`)
    ...
    && !(entity.concordances?.osm || [entity.osm]).some((osm) => existingOsm.has(`${osm.type}:${osm.id}`)))
...
await writeFile(dataPath, `// Generated from explicitly approved cached map-data reviews.\nexport const ${exportName(options.country)} = Object.freeze(${JSON.stringify(deduped, null, 2)});\n`);
